...
function downloadPhoto(applicationId) {
    ...
    if (application && application.photo) {
        try {
            ...
            const byteString = atob(application.photo.split(',')[1]);
            const mimeString = application.photo.split(',')[0].split(':')[1].split(';')[0];
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `employee_photo_${application.uni_id}.jpg`;
            ...
            alert('حدث خطأ أثناء تحميل الصورة / Error downloading photo');
        }
    } else {
        alert('الصورة غير متوفرة / Photo not available');
    }
}
...
function viewFullPhoto(photoUrl) {
    ...
    modal.innerHTML = `
        <div class="photo-modal-content">
            ...
            <img src="${photoUrl}" alt="Employee Photo" class="full-photo">
        </div>
    `;
