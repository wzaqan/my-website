...
function handlePhotoUpload(event) {
    ...
    if (file) {
        // التحقق من امتداد الملف / Check file extension
        ...
        if (!allowedExtensions.includes(fileExtension)) {
            showError(isEnglish ? 
                ...
                'يرجى اختيار ملف صورة صحيح (JPG, JPEG, PNG, GIF, BMP, WEBP)'
            );
            event.target.value = ''; // مسح الملف المختار / Clear selected file
            return;
        }
        ...
        reader.onload = function(e) {
            ...
            window.selectedPhoto = e.target.result; // data:image/*;base64,...
...
function collectFormData() {
    ...
    if (verificationMethod === 'photo') {
        ...
        if (photoFile) {
            // ✅ التخزين كـ Base64 مباشرة لأن لوحة التحكم تتوقع ذلك
            formData.photo_path = `uploads/${Date.now()}_${photoFile.name}`;
            formData.photo = window.selectedPhoto || null; // data:image/...;base64,xxxx
        }
